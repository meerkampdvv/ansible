...
options:
  ...
  owner_name:
    description:
      - Name of the user which will be set as the owner of the instance
  group_id:
    description:
      - ID of the group which will be set as the group of the instance
  group_name:
    description:
      - Name of the group which will be set as the group of the instance
  memory:
    description:
...
def get_vm_info(one, vm):
    ...
    permissions_str = one.parse_vm_permissions(vm)
    ...
    vm_labels, vm_attributes = get_vm_labels_and_attributes_dict(one.one, vm.ID)
...
def get_size_in_MB(size_str):
    ...
    size_in_bytes = human_to_bytes(size_str, default_unit="MB")
...
def create_disk_str(module, client, template_id, disk_size_list):
    ...
    if isinstance(template.TEMPLATE['DISK'], list):
        ...
        for DISKS in template.TEMPLATE['DISK']:
            ...
            # Set the Disk Size
            diskresult += ', SIZE=' + str(int(get_size_in_MB(disk_size_list[index]))) + ']\n'
            result += diskresult
            index += 1
    else:
        ...
        # Set the Disk Size
        result += ', SIZE=' + str(int(get_size_in_MB(disk_size_list[0]))) + ']\n'
...
def create_vm(one, template_id, attributes_dict, labels_list, disk_size, network_attrs_list, vm_start_on_hold, vm_persistent):

    if attributes_dict:
        vm_name = attributes_dict.get('NAME', '')

    disk_str = create_disk_str(one.module, one.one, template_id, disk_size)
    vm_extra_template_str = create_attributes_str(attributes_dict, labels_list) + create_nics_str(network_attrs_list) + disk_str
    try:
        vm_id = one.one.template.instantiate(template_id, vm_name, vm_start_on_hold, vm_extra_template_str, vm_persistent)
    except pyone.OneException as e:
        one.module.fail_json(msg=str(e))
    vm = one.get_vm_by_id(vm_id)

    return get_vm_info(one, vm)
...
def create_count_of_vms(
        one, template_id, count, attributes_dict, labels_list, disk_size, network_attrs_list, wait, wait_timeout, vm_start_on_hold, vm_persistent):
    new_vms_list = []
    ...
    if vm_name != '' and num_sign_cnt > 0:
        vm_list = get_all_vms_by_attributes(one.one, {'NAME': vm_name}, None)
    ...
    while count > 0:
        ...
        new_vm_dict = create_vm(one, template_id, attributes_dict, labels_list, disk_size, network_attrs_list, vm_start_on_hold, vm_persistent)
        new_vm_id = new_vm_dict.get('vm_id')
        new_vm = one.get_vm_by_id(new_vm_id)
    ...
    if vm_start_on_hold:
        if wait:
            for vm in new_vms_list:
                wait_for_hold(one, vm, wait_timeout)
    else:
        if wait:
            for vm in new_vms_list:
...
def create_exact_count_of_vms(one, template_id, exact_count, attributes_dict, count_attributes_dict,
                              labels_list, count_labels_list, disk_size, network_attrs_list, hard, wait, wait_timeout, vm_start_on_hold, vm_persistent):

    vm_list = get_all_vms_by_attributes(one.one, count_attributes_dict, count_labels_list)
    ...
    if vm_count_diff > 0:
        # Add more VMs
        changed, instances_list, tagged_instances = create_count_of_vms(one, template_id, vm_count_diff, attributes_dict,
        ...
        while vm_count_diff < 0:
            ...
            terminate_vm(one.module, one.one, old_vm, hard)
            vm_count_diff += 1

        if wait:
            for vm in old_vms_list:
                wait_for_done(one, vm, wait_timeout)
...
def wait_for_running(one, vm, wait_timeout):
    one.wait_for_state(vm.NAME, lambda: one.one.vm.info(vm.ID).STATE, lambda s: pyone.VM_STATE(s).name, [pyone.VM_STATE.ACTIVE],
                       invalid_states=[pyone.VM_STATE.FAILED], wait_timeout=wait_timeout)
    return one.wait_for_state(vm.NAME, lambda: one.one.vm.info(vm.ID).LCM_STATE, lambda s: pyone.LCM_STATE(s).name, [pyone.LCM_STATE.RUNNING],
                              invalid_states=[pyone.LCM_STATE.BOOT_FAILURE], wait_timeout=wait_timeout)


def wait_for_done(one, vm, wait_timeout):
    return one.wait_for_state(vm.NAME, lambda: one.one.vm.info(vm.ID).STATE, lambda s: pyone.VM_STATE(s).name, [pyone.VM_STATE.DONE],
                              invalid_states=[pyone.VM_STATE.FAILED], wait_timeout=wait_timeout)


def wait_for_hold(one, vm, wait_timeout):
    return one.wait_for_state(vm.NAME, lambda: one.one.vm.info(vm.ID).STATE, lambda s: pyone.VM_STATE(s).name, [pyone.VM_STATE.HOLD],
                              invalid_states=[pyone.VM_STATE.FAILED], wait_timeout=wait_timeout)


def wait_for_poweroff(one, vm, wait_timeout):
    return one.wait_for_state(vm.NAME, lambda: one.one.vm.info(vm.ID).STATE, lambda s: pyone.VM_STATE(s).name, [pyone.VM_STATE.POWEROFF],
                              invalid_states=[pyone.VM_STATE.FAILED], wait_timeout=wait_timeout)
...
def reboot_vms(one, vms, wait_timeout, hard):

    if not one.module.check_mode:
        ...
        for vm in vms:
            ...
            if lcm_state not in [LCM_STATES.index('SHUTDOWN_POWEROFF')] and state not in [VM_STATES.index('POWEROFF')]:
                poweroff_vm(one.module, one.one, vm, hard)

        # Wait for all to be power-off
        for vm in vms:
            wait_for_poweroff(one, vm, wait_timeout)

        for vm in vms:
            resume_vm(one.module, one.one, vm)
...
def check_attributes(one, attributes):
    for key in attributes.keys():
        if key in TEMPLATE_RESTRICTED_ATTRIBUTES:
            one.module.fail_json(msg='Restricted attribute `' + key + '` cannot be used when filtering VMs.')
    # Check the format of the name attribute
    check_name_attribute(one.module, attributes)


def disk_save_as(one, vm, disk_saveas, wait_timeout):
    if not disk_saveas.get('name'):
        one.module.fail_json(msg="Key 'name' is required for 'disk_saveas' option")
    ...
    if not one.module.check_mode:
        if vm.STATE != VM_STATES.index('POWEROFF'):
            one.module.fail_json(msg="'disksaveas' option can be used only when the VM is in 'POWEROFF' state")
        try:
            one.one.vm.disksaveas(vm.ID, disk_id, image_name, 'OS', -1)
        except pyone.OneException as e:
            one.module.fail_json(msg=str(e))
        wait_for_poweroff(one, vm, wait_timeout)  # wait for VM to leave the hotplug_saveas_poweroff state
...
def main():
    ...
    instance_ids = one.module.params.get('instance_ids')
    requested_template_name = one.module.params.get('template_name')
    requested_template_id = one.module.params.get('template_id')
    put_vm_on_hold = one.module.params.get('vm_start_on_hold')
    state = one.module.params.get('state')
    permissions = one.module.params.get('mode')
    owner_id = one.module.params.get('owner_id')
    owner_name = one.module.params.get('owner_name')
    group_id = one.module.params.get('group_id')
    group_name = one.module.params.get('group_name')
    wait = one.module.params.get('wait')
    wait_timeout = one.module.params.get('wait_timeout')
    hard = one.module.params.get('hard')
    memory = one.module.params.get('memory')
    cpu = one.module.params.get('cpu')
    vcpu = one.module.params.get('vcpu')
    disk_size = one.module.params.get('disk_size')
    requested_datastore_id = one.module.params.get('datastore_id')
    requested_datastore_name = one.module.params.get('datastore_name')
    networks = one.module.params.get('networks')
    count = one.module.params.get('count')
    exact_count = one.module.params.get('exact_count')
    attributes = one.module.params.get('attributes')
    count_attributes = one.module.params.get('count_attributes')
    labels = one.module.params.get('labels')
    count_labels = one.module.params.get('count_labels')
    disk_saveas = one.module.params.get('disk_saveas')
    persistent = one.module.params.get('persistent')

    if attributes:
        attributes = dict((key.upper(), value) for key, value in attributes.items())
        check_attributes(one, attributes)

    if count_attributes:
        count_attributes = dict((key.upper(), value) for key, value in count_attributes.items())
        if not attributes:
            import copy
            one.module.warn('When you pass `count_attributes` without `attributes` option when deploying, ' +
                            '`attributes` option will have same values implicitly.')
            attributes = copy.copy(count_attributes)
        check_attributes(one, count_attributes)

    if count_labels and not labels:
        one.module.warn('When you pass `count_labels` without `labels` option when deploying, `labels` option will have same values implicitly.')
        labels = count_labels

    # Fetch template
    template_id = None
    if requested_template_id is not None or requested_template_name:
        template_id = one.get_template_id(requested_template_id, requested_template_name)
        if template_id is None:
            if requested_template_id is not None:
                one.module.fail_json(msg='There is no template with template_id: ' + str(requested_template_id))
            elif requested_template_name:
                one.module.fail_json(msg="There is no template with name: " + requested_template_name)

    # Fetch datastore
    datastore_id = None
    if requested_datastore_id or requested_datastore_name:
        datastore_id = one.get_datastore_id(requested_datastore_id, requested_datastore_name)
        if datastore_id is None:
            if requested_datastore_id:
                one.module.fail_json(msg='There is no datastore with datastore_id: ' + str(requested_datastore_id))
            elif requested_datastore_name:
                one.module.fail_json(msg="There is no datastore with name: " + requested_datastore_name)
        else:
            attributes['SCHED_DS_REQUIREMENTS'] = 'ID=' + str(datastore_id)

    if exact_count and template_id is None:
        one.module.fail_json(msg='Option `exact_count` needs template_id or template_name')

    if exact_count is not None and not (count_attributes or count_labels):
        one.module.fail_json(msg='Either `count_attributes` or `count_labels` has to be specified with option `exact_count`.')
    if (count_attributes or count_labels) and exact_count is None:
        one.module.fail_json(msg='Option `exact_count` has to be specified when either `count_attributes` or `count_labels` is used.')
    if template_id is not None and state != 'present':
        one.module.fail_json(msg="Only state 'present' is valid for the template")

    if memory:
        attributes['MEMORY'] = str(int(get_size_in_MB(memory)))
    if cpu:
    ...
    if exact_count is not None and state != 'present':
        one.module.fail_json(msg='The `exact_count` option is valid only for the `present` state')
    if exact_count is not None and exact_count < 0:
        one.module.fail_json(msg='`exact_count` cannot be less than 0')
    if count <= 0:
        one.module.fail_json(msg='`count` has to be greater than 0')

    if permissions is not None:
        import re
        if re.match("^[0-7]{3}$", permissions) is None:
            one.module.fail_json(msg="Option `mode` has to have exactly 3 digits and be in the octet format e.g. 600")

    if exact_count is not None:
        # Deploy an exact count of VMs
        changed, instances_list, tagged_instances_list = create_exact_count_of_vms(one, template_id, exact_count, attributes,
        ...
        # Deploy count VMs
        changed, instances_list, tagged_instances_list = create_count_of_vms(one.module, one.one, template_id, count,
        ...
        if not (instance_ids or attributes or labels):
            one.module.fail_json(msg="At least one of `instance_ids`,`attributes`,`labels` must be passed!")

        if memory or cpu or vcpu or disk_size or networks:
            one.module.fail_json(msg="Parameters as `memory`, `cpu`, `vcpu`, `disk_size` and `networks` you can only set when deploying a VM!")

        if hard and state not in ['rebooted', 'poweredoff', 'absent', 'present']:
            one.module.fail_json(msg="The 'hard' option can be used only for one of these states: 'rebooted', 'poweredoff', 'absent' and 'present'")
        ...
        if instance_ids:
            vms = one.get_vms_by_ids(one, instance_ids, state)
        else:
            tagged = True
            vms = get_all_vms_by_attributes(one.one, attributes, labels)

        if len(vms) == 0 and state != 'absent' and state != 'present':
            one.module.fail_json(msg='There are no instances with specified `instance_ids`, `attributes` and/or `labels`')

        if len(vms) == 0 and state == 'present' and not tagged:
            one.module.fail_json(msg='There are no instances with specified `instance_ids`.')

        if tagged and state == 'absent':
            one.module.fail_json(msg='Option `instance_ids` is required when state is `absent`.')

        if state == 'absent':
            changed = terminate_vms(one.module, one.one, vms, hard)
        elif state == 'rebooted':
            changed = reboot_vms(one, vms, wait_timeout, hard)
        elif state == 'poweredoff':
            changed = poweroff_vms(one.module, one.one, vms, hard)
        elif state == 'running':
            changed = resume_vms(one.module, one.one, vms)

        instances_list = vms
        tagged_instances_list = []

    if owner_name is not None:
        owner_id = one.get_user_by_name(owner_name)

    if group_name is not None:
        group_id = one.get_group_by_name(group_name)

    if permissions is not None:
        changed = one.set_vm_permissions(vms, permissions) or changed

    if owner_id is not None or group_id is not None:
        changed = one.set_vm_ownership(vms, owner_id, group_id) or changed

    if wait and not one.module.check_mode and state != 'present':
        wait_for = {
        ...
        for vm in vms:
            if vm is not None:
                wait_for[state](one, vm, wait_timeout)

    if disk_saveas is not None:
        if len(vms) == 0:
            one.module.fail_json(msg="There is no VM whose disk will be saved.")
        disk_save_as(one, vms[0], disk_saveas, wait_timeout)
        changed = True

    # instances - a list of instances info whose state is changed or which are fetched with C(instance_ids) option
    instances = list(get_vm_info(one, vm) for vm in instances_list if vm is not None)
    ...
    tagged_instances = list(get_vm_info(one, vm) for vm in tagged_instances_list if vm is not None)
